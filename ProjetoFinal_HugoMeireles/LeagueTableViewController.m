...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section{
    return [NSString stringWithFormat:@"Posição |  Nome   |   Jogos   |  Pontos"];
}
...
- (void)showAlert:(LeaguePosition *)team League:(NSString *)teamLeague {
    ...
    UIAlertAction *action4 = [UIAlertAction actionWithTitle:@"Ver clube" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self performSegueWithIdentifier:@"goToClubProfile" sender:team];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    }else if ([segue.destinationViewController isKindOfClass:TeamProfileViewController.class]){
        LeaguePosition *leaguePosition = sender;
        TeamProfileViewController *teamvc = segue.destinationViewController;
        teamvc.teamId = leaguePosition.teamId;
        teamvc.teamName = leaguePosition.teamName;
        teamvc.title = leaguePosition.teamName;
    }
}
