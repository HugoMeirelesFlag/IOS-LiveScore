...
- (void)showAlert:(FavoriteMO *)favorite{
    ...
    UIAlertAction *action4 = [UIAlertAction actionWithTitle:@"Ver clube" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self performSegueWithIdentifier:@"goToClubProfile" sender:favorite];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.destinationViewController isKindOfClass:TeamMatchesTableViewController.class]){
        ...
        teamvc.teamId = @(favorite.teamId);
        teamvc.teamName = favorite.teamName;
        teamvc.title = favorite.teamName;
    }
}
