...
- (void)getAllLeagueMatcheswithId:(NSNumber *)leagueId withMatchDay:(NSNumber *)matchDay completionBlock:(void (^)(NSDictionary *, NSError *))completion{
    ...
    NSString *leagueUrlId;
    if ((matchDay.integerValue > 6 && leagueId.intValue == 2000) || (matchDay.integerValue > 6 && leagueId.intValue == 2001) || (matchDay.integerValue > 6 && leagueId.intValue == 2018) ) {
        leagueUrlId = [NSString stringWithFormat:@"https://api.football-data.org/v2/competitions/%@/matches?stage=%@",leagueId,[Tornament getStage:matchDay]];
    }else{
        leagueUrlId = [NSString stringWithFormat:@"https://api.football-data.org/v2/competitions/%@/matches?matchday=%@",leagueId,matchDay];
    }
    NSURL *url = [NSURL URLWithString:leagueUrlId];
